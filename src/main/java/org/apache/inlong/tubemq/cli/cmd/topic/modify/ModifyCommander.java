...
package org.apache.inlong.tubemq.cli.cmd.topic.modify;
...
@Command(name = "modify", description = "modify existed topic")
public class ModifyCommander implements Runnable {
    ...
    @Option(names = {"-p", "--publish"}, description = "the state of accept publish")
    String acceptPublish;

    @Option(names = {"--modify-date"}, description = "the date of modify", defaultValue = "default")
    String modifyDate;

    @Option(names = {"-s", "--subscribe"}, description = "the state of accept subscribe")
    String acceptSubscribe;
    ...
    public void run() {
        ...
        builder.setType("op_modify")
                ...
                .setParamsIfNotEmpty("acceptPublish", acceptPublish)
                .setParamsIfNotEmpty("acceptSubscribe", acceptSubscribe)
                .setKey()
                .setParamsMap(params);
        ...
        try {
            ...
            Logger.logInfo("Modified topic success!");
            Logger.logBlank();
            for (Datum entry : result.getData()) {
                Logger.logInfo(
                        "Modified topic name \"" + entry.getTopicName() + "\" in brokerId: " + entry.getBrokerId());
            }
        } catch (IOException e) {
            Logger.logError("Error when sending packet to TubeMQ-Server: Modify Topic");
